...
function SkillItem({ name, index, input, setInput, setMode, setEditingIndex }){
    function onDelete(e){
        ...
        let temp = input.skills.filter((skill) => skill !== e.target.previousSibling.previousSibling.textContent)
    ...
    return(
        <div>
            <h3>{name}</h3>
            <button onClick={() => {setMode("edit"); setEditingIndex(index)}}>Edit</button>
        ...
        <div>
            ...
            <ul>
                {input.skills.map((skill, index) => {
                    return <SkillItem name={skill} index={index} input={input} setInput={setInput} setMode={setMode} setEditingIndex={setEditingIndex}/>
                })}
            </ul>
...
export default function SkillsForm({ input, setInput }){
    ...
    if(mode == "form"){
        return (
            <RenderForm input={input} setInput={setInput} setMode={setMode} setEditingIndex={setEditingIndex}/>
        )
    }else if(mode == "add"){
        return (
        ...
        return (
            <RenderEditSkill editingIndex={editingIndex} input={input} setMode={setMode} setInput={setInput}/>
        )
    }
}